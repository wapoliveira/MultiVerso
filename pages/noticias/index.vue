<script setup>
useHead({
    title: 'MultiVerso - Minhas Noticias',
})

definePageMeta({
    layout: "default",
    name: "MultiVerso - Minhas Noticias",
})
...
const listNews = async () => {
    try {
        const response = await fetch(`/api/news?user_id=${userStore?.userData?.id}`)
                        ...
                        <tbody>
                            <tr v-for="(newData, index) in news" :key="newData.id" class="odd:bg-white even:bg-gray-200 border-b">
                                ...
                                <td class="px-6 py-4 flex">
                                    <NuxtLink :to="'/noticias/novo/'+newData?.id" v-if="newData?.status == 'rejected' || newData?.status == 'pending'">
                                        <Icon name="material-symbols:edit-square" class="text-gray-500 cursor-pointer hover:text-orange-500 h-10 mr-2" />
                                    </NuxtLink>
                                    <Icon v-if="newData?.status == 'pending'" @click="isOpenDelete = true, key++, newsData = newData" name="material-symbols:delete-rounded" class="text-gray-500 cursor-pointer hover:text-orange-500 h-10" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
